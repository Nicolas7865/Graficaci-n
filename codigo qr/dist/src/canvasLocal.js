...
export class CanvasLocal {
    ...
    drawQrModule(col, row, isBlack, color = '#000') {
    ...
    paintAnatomy() {
        this.clearCanvasArea();
        const N = this.qrSize;
        let qrMatrix = Array.from({ length: N }, () => Array(N).fill('white')); 

        const colorFinder = 'blue';                 
        const colorSeparator = 'lightblue';         
        const colorAlignment = 'purple';            
        const colorTiming = 'orange';               
        const colorFormatInfo = 'red';              
        const colorVersionInfo = 'darkgreen';       
        const colorFixedBlack = 'grey';             
        const colorData = 'yellow';                 
        const colorECC = 'pink';                    
        ...
        const placeFinder = (colStart, rowStart) => {
            for (let r = 0; r < 7; r++) {
                for (let c = 0; c < 7; c++) {
        ...
        const placeSeparator = (colStart, rowStart) => {
            for (let i = -1; i < 8; i++) {
                if (rowStart > 0 && colStart + i >= 0 && colStart + i < N) qrMatrix[rowStart - 1][colStart + i] = colorSeparator; // Top
                if (rowStart + 7 < N && colStart + i >= 0 && colStart + i < N) qrMatrix[rowStart + 7][colStart + i] = colorSeparator; // Bottom
            }
            for (let i = 0; i < 7; i++) {
                if (colStart > 0 && rowStart + i >= 0 && rowStart + i < N) qrMatrix[rowStart + i][colStart - 1] = colorSeparator; // Left
                if (colStart + 7 < N && rowStart + i >= 0 && rowStart + i < N) qrMatrix[rowStart + i][colStart + 7] = colorSeparator; // Right
            }
        };
        placeSeparator(0, 0);
        ...
        if (N >= 25) {
            for (let r = 0; r < 5; r++) {
                for (let c = 0; c < 5; c++) {
                    ...
                    else qrMatrix[alignTopLeft + r][alignTopLeft + c] = 'white'; // Inner white part
                }
            }
        }

        this.alwaysOneCoord.forEach(linearIdx => {
            const r = Math.floor(linearIdx / N);
            const c = linearIdx % N;
        ...
        for (const blockKey in this.dataModuleIndices) {
            const blockPositions = this.dataModuleIndices[blockKey];
            for (const linearIndex of blockPositions) {
                const r = Math.floor(linearIndex / N);
                const c = linearIndex % N;
                if (qrMatrix[r][c] === 'white') {
